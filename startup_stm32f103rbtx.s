...
.equ DELAY_D1, 51870	@50540, взято 19,5
.equ DELAY_W1, 1330		@2660 - 1 ms, взято 0,5
.equ DELAY_D2, 46550	@47880, взято 17,5
.equ DELAY_W2, 6650		@5320 - 2 ms, взято 2,5
.equ BUT_DELAY, 0x111FFF
...
/* Call the clock system initialization function.*/
    ...
    //подключаем тактирование кнопки

@    mov32	r0, PERIPH_BB_BASE+ \
				(RCC_APB2ENR-PERIPH_BASE)*32 + \
				4*4
	//загружаем адрес бита порта С для тактирования
@	mov r1, #1
@	str r1, [r0]

	mov32 r0, GPIOC_CRH
	mov r1, #0x04	@маска "0100" - Floating input (reset state), Input mode (reset state)
	ldr r2, [r0]
	bfi r2, r1, #20, #4
	str r2, [r0]

	@начинаем работать с диодиком и кнопкой

    mov32	r0, GPIOC_BSRR				@ адрес порта выходных сигналов диода
	ldr		r4, =DELAY_D1
	ldr		r5, =DELAY_W1
	mov		r6, #1						@задаем начальный статус 1 поворота
...
loop:									@ Бесконечный цикл
	...
	mov32	r1, #GPIO_BSRR_BR7			@ сбрасываем в '0'
	str 	r1, [r0]					@ загружаем в порт

	mov r10, r4
	bl		delay						@ задержка

	ldr r3, =GPIOC_IDR					@считываем значения на выводах порта кнопки
	ldr r1, [r3]
	mov r2, #GPIO_IDR_IDR13
	ANDS r1, r2
	it EQ
	bleq btn
...
delay:									@ Подпрограмма задержки
	...
	mov	r0, r10		@ псевдоинструкция Thumb (загрузить константу в регистр)
delay_loop:
...
btn:
	push	{r0}
	subs	r6, #1					@вычитаем из статуса 1, если 0, то это статус 1, если не 0, то это статус 2
	it		eq
	beq		status1					@если выполнилось условие, то переходим к метке status1

	@если не выполнилось условие, то продолжаем делать дальше status2
	ldr		r4, =DELAY_D1
	ldr		r5, =DELAY_W1
...
//	mov32	r1, #GPIO_BSRR_BS5			@ сбрасываем в '0'
//	str 	r1, [r0]					@ загружаем в порт
	pop 	{r0}
	bx lr
